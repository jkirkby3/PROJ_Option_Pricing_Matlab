%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% MONTE CARLO RETURN TIMER OPTION PRICER for Diffusions AND Jump Diffusions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Descritpion: Script to Price Return Timer options in Diffusion and Jump Diffusion Models
...
% l      = -2.0;   % Lower return Barrier 
% u      = 0.25;

call   = 0;    % For call use 1 (else, its a put)
l      = -0.25;   % Lower return Barrier 
u      = 2.0;
...
moneyness = [0.5 0.6 0.7 0.8 .85 .90 .95 1 1.05 1.10 1.15  1.20 1.3 1.4 1.5];
Kvec   = S_0*moneyness;   % strikes to price
%Kvec = [100];
...
[prices, stdErrs] = Price_MC_Return_Timer_Strikes_func(Spath, call, l, u, Kvec, M, mult, r, T);

[prices_no_bar, stdErrs_no_bar] = Price_MC_Return_Barrier_Strikes_func(Spath, call, -10000, 100000, Kvec, M, mult, rebate, r, T);
% Plot
h=figure();
plot(moneyness, prices, 'linewidth', 1.2)
hold on;
plot(moneyness, prices_no_bar, 'linewidth', 1.2)
legend({'ReturnTimer', 'European'},'interpreter','latex')
...
h2=figure();
plot(moneyness, 100*(prices_no_bar - prices)./prices_no_bar, 'linewidth', 1.2)
legend({'Percentage Point Diff'},'interpreter','latex')
