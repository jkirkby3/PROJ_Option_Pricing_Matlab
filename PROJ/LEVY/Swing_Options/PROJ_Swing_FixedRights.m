...
% function price = FixedRightsPrice_func( M,K,alpha,rnCHF,r,Dmax,T_0,T,Ns,S_0,Ks)
% %  
% %   FOR NOW: T_0 must = 0
% %
% 
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% %%% dont include this first step in timing since we recalc later for ease
% %%% of coding
% %%%----------------------------------------
% G      = Dmax*Gx(xmin + dx*(0:K-1))';
% %%%----------------------------------------
...
% Cons4  = 1/12;  
% %%%----------------------------------------
% varths_dt = zeros(1,4);  %%% vartheta's
% gamms  = zeros(1,3);
% psis   = zeros(1,4);  %%% psi's
% 
% zetas  = zeros(1,2);
% xbars  = zeros(1,2);
% xnbars = zeros(1,2);
...
%         if n-1 > M -m %in these states it is always optimal to exercise
%             THET(:,n) = ThetaDmax;
%             THET(2:K-1,n) = THET(2:K-1,n) + Cons4*(CONT_dt(1:K-2,n-1) + 10*CONT_dt(2:K-1,n-1) + CONT_dt(3:K,n-1));
%             nm1vec(n) = nbars(2); nm2vec(n) = nbars(3);
%         else %n -1 <= M - m  %we added 1 to each state n
%             
%              %nms(1) = nbars(2); nms(2) = nbars(3) +1; 
%             nms(1) = nm1vec(n); nms(2) = nm2vec(n) +1;
%             while CONT_dt(nms(1),n)> PSI(nms(1))  %%% If they are equal to start, then nms(1) = nbars(2)
%                 nms(1) = nms(1) -1;
%             end
% 
%             while nms(2)<K-1 && CONT_dt(nms(2),n)> PSI(nms(2)) %%%% less than K-1 so the algorithm works in extreme cases
%                 nms(2) = nms(2) + 1;
%             end
%             nms(2) = nms(2) -1;
%             
...
% 
%             %%%----------------------------------------
%             tmp1     = CONT_dt(nms(1),n)-PSI(nms(1));      tmp2 = CONT_dt(nms(1)+1,n) - PSI(nms(1)+1);
%             xbars(1) = xnbars(1) +max(0,dx*tmp1/(tmp1-tmp2));
%             
%             
%             %%%------------------
%             tmp1     = CONT_dt(nms(2),n)-PSI(nms(2));      tmp2 = CONT_dt(nms(2)+1,n) - PSI(nms(2)+1);
%             xbars(2) = xnbars(2) +max(0,dx*tmp1/(tmp1-tmp2));
%         
%             %%%------------------
%             rhos      = xbars - xnbars;
...
% 
%             psis          = Get_psis_swing( rhos,zetas, q_plus, q_minus, Ks, a , varthet_01, E,nms, nbars,edn);
%             varths_dt_n   = Get_Varths_swing(zetas,nms,CONT_dt(:,n));
%             varths_dt_nm1 = Get_VarthsDD_swing(zetas,nms,CONT_dt(:,n-1));
%             
%             %%%----------------------------------------
%             THET(1,n)          = PSI(1);   %%%% For simplicity... won't affect solution
%             THET(2:nms(1)-1,n) = ThetaDmax(2:nms(1)-1) + Cons4*(CONT_dt(1:nms(1)-2,n-1) + 10*CONT_dt(2:nms(1)-1,n-1) + CONT_dt(3:nms(1),n-1));
% 
%             THET(nms(1),n)     = ThetaDmax(nms(1)) - Dmax*psis(1) + varths_dt_n(1)    + varths_dt_nm1(3);
%             THET(nms(1)+1,n)   = Dmax*psis(2) + varths_dt_n(2)    + varths_dt_nm1(4);
% 
%             THET(nms(1)+2: nms(2)-1,n) = Cons4*(CONT_dt(nms(1)+1: nms(2)-2,n) + 10*CONT_dt(nms(1)+2: nms(2)-1,n) + CONT_dt(nms(1)+3: nms(2),n));
% 
%             THET(nms(2),n)       = Dmax*psis(3) + varths_dt_n(3)     + varths_dt_nm1(1);
%             THET(nms(2)+1,n)     = ThetaDmax(nms(2)+1) - Dmax*psis(4) + varths_dt_n(4)     + varths_dt_nm1(2);
% 
%             THET(nms(2)+2:K-1,n) = ThetaDmax(nms(2)+2:K-1)  + Cons4*(CONT_dt(nms(2)+1:K-2,n-1) + 10*CONT_dt(nms(2)+2:K-1,n-1) + CONT_dt(nms(2)+3:K,n-1));
%             THET(K,n)            = PSI(K); %%%% For simplicity... won't affect solution
%         end
%         
%         
...
% end
% 
% PSI = G + CONT_dt(:,Ns);  %%% correspons to Ns-1
% 
% xnot = xmin +(nnot-1)*dx;
% % xs   = [xnot-2*dx,xnot-dx,xnot,xnot+dx,xnot+2*dx];
% % inds = [nnot-2,nnot-1,nnot,nnot+1,nnot+2];
% xs   = [xnot-dx,xnot,xnot+dx,xnot+2*dx];
% inds = [nnot-1,nnot,nnot+1,nnot+2];
% ys   = max(PSI(inds),CONT_dt(inds,Ns+1));
% 
% 
% price = spline(xs,ys,0);
% 
